...
class MainPage(webapp.RequestHandler):
  ...
  def get(self):
    ...
    if not self.request.get('query'):
      ...
      # the first variant is just escape
      # query = cgi.escape(query);

      # the second variant: using regEx
      xssMatch = re.search("<script>.*<\/script>", query, re.IGNORECASE);

      if xssMatch:
        message = 'XSS attack was prevented!'
      else:
